...
export class DatabaseManager {
   ...
   * 创建密码本
   */
  public async createPasswordBook(name: string, spaceId: string, description: string): Promise<Types.PasswordBook> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
   ...
   * 更新密码本
   */
  public async updatePasswordBook(bookId: string, name: string, description: string): Promise<Types.PasswordBook> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
  ...
  public async createPassword(
    ...
    stmt.run(id, bookId, username, encrypted.encrypted, encrypted.iv, masked, url, description, newSortOrder);
   ...
   * 批量更新密码排序
   */
  public async batchUpdatePasswordSortOrder(sortOrders: {id: string, sortOrder: number}[]): Promise<boolean> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
