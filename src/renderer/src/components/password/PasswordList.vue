    ...
    <div v-else>
      ...
      >
        ...
        <el-table-column label="操作" width="240">
          <template #default="scope">
            <el-button size="small" type="success" @click="$emit('edit-password', scope.row)" circle>
              <el-icon><Edit /></el-icon>
            </el-button>
            <el-button size="small" type="primary" :disabled="!scope.row.url" @click="openExternal(scope.row.url)" circle>
              <el-icon><Link /></el-icon>
            </el-button>
            <el-button size="small" type="danger" @click="$emit('del-password', scope.row)" circle>
              <el-icon><Delete /></el-icon>
            </el-button>
            <el-button size="small" @click="moveUp(scope.$index)" :disabled="scope.$index === 0" circle><el-icon><ArrowUpBold /></el-icon></el-button>
            <el-button size="small" @click="moveDown(scope.$index)" :disabled="scope.$index === localPasswords.length - 1" circle><el-icon><ArrowDownBold /></el-icon></el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
import {InfoFilled, Plus, CopyDocument, Link, Delete, Edit, ArrowUpBold, ArrowDownBold} from '@element-plus/icons-vue'
...
// 监听父组件传入的密码数据变化
watch(() => props.passwords, (newPasswords) => {
...
const updateSortOrder = async () => {
  try {
    ...
    const result = await window.api.batchUpdatePasswordSort(
      props.spaceId,
      props.bookId,
      sortOrders
    )
