...
export class PasswordCrypto {
  ...
  private deriveSpecializedKey(masterKey: Buffer, purpose: string, length: number = 32): Buffer {
    return crypto.hkdfSync('sha256', masterKey, '', purpose, length);
  }
  ...
  async initialize(masterPassword: string, salt: string): Promise<void> {
    try {
      this.salt = salt;
      const masterKey = await this.deriveMasterKey(masterPassword, salt, this.keyDerivationIterations);
  ...
  private isCommonPassword(password: string): boolean {
    const commonPasswords = [
      ...
      'passw0rd', 'master', 'hello', 'login', 'princess'
    ];
  ...
  maskPassword(
    str: string,
    visiblePrefix: number = 2,
    visibleSuffix: number = 2
  ): string {
...
export async function demo() {
  ...
  // 6. 解密用户密码
  const decryptedPassword = crypto.decrypt(
